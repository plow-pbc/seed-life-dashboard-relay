...
- The install action MUST land a `DASHBOARD_TOKEN` on prod. Resolution order:
  ...
  2. **Already set (no-env redeploy)** — only when NO env value is supplied: if `DASHBOARD_TOKEN` is already present on prod, reuse it when its value is readable via `vercel env pull`; a write-only Sensitive var returns nothing from the pull, so the action recovers the prior token from the state file when one exists — else rotates to a fresh `openssl rand -hex 32` — pushed via the same rm-then-add as rule 1, instead of aborting (see the redeploy note below).
...
- A **no-env** redeploy MUST NOT regenerate a readable `DASHBOARD_TOKEN`: the operator sees a "token already set on Vercel — reusing" message and the value is pulled via `vercel env pull` — BEFORE the deploy — so the state file stays in sync. Exception: a write-only Sensitive var cannot be pulled, so the action recovers the prior token from the state file when one exists (no rotation), else generates a fresh one — pushed before `vercel deploy --prod` so the running deployment and the state file both carry the same value. (An env-supplied redeploy instead overwrites per rule 1.)
