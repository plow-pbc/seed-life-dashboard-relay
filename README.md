...
4. Resolves a `DASHBOARD_TOKEN` (the bearer the relay validates on every `/api/message` write/read): an env-supplied value is authoritative; otherwise every deploy mints a fresh one via `openssl rand -hex 32` (consumers pick the new value up from the state file — see the retired rotation rule in git history: `git log -- SEED.md`).
