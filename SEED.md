...
- A no-env redeploy resolves `DASHBOARD_TOKEN` BEFORE `vercel deploy --prod`, so the running deployment and the state file carry the same value; Sensitive recovery/rotation follows rule 2. Note: a dashboard-side manual rotation of a Sensitive `DASHBOARD_TOKEN` is therefore reverted by the next no-env redeploy — supply the new value via env (rule 1) to make it stick.
