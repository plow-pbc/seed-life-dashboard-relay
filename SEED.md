...
- The install action MUST `git clone --depth 1 --branch "$SEED_BRANCH"` (default `main`) the viewer repo to a per-household cache under `~/Library/Caches/seed-life-dashboard-relay/source/`; re-runs `git fetch --depth=1 origin "$SEED_BRANCH"` + `git reset --hard FETCH_HEAD` rather than re-cloning. There is NO fallback to `main`: a `SEED_BRANCH` absent on the viewer remote fails loudly rather than silently building a mixed-source install.
