...
- The install action MUST land a `DASHBOARD_TOKEN` on prod. Resolution order:
  ...
  2. **Already set (no-env redeploy)** — only when NO env value is supplied: if `DASHBOARD_TOKEN` is already present on prod, reuse it when its value is readable via `vercel env pull`; a write-only Sensitive var returns nothing from the pull, so the action recovers the prior token from the state file when one holds a usable value — else rotates to a fresh `openssl rand -hex 32` — pushed via the same rm-then-add as rule 1, instead of aborting. The re-add is a plain readable var: the Sensitive marking is intentionally dropped so future redeploys can pull the value back (see the redeploy note below).
...
- A **no-env** redeploy MUST NOT regenerate a readable `DASHBOARD_TOKEN`: the operator sees a "token already set on Vercel — reusing" message and the value is pulled via `vercel env pull` — BEFORE the deploy — so the state file stays in sync. Exception: a write-only Sensitive var cannot be pulled, so the action recovers the prior token from the state file when one holds a usable value (no rotation), else generates a fresh one — pushed before `vercel deploy --prod` so the running deployment and the state file both carry the same value, re-added as a plain readable var (Sensitive marking intentionally dropped). Note: a dashboard-side manual rotation of a Sensitive `DASHBOARD_TOKEN` is therefore reverted by the next no-env redeploy — supply the new value via env (rule 1) to make it stick. (An env-supplied redeploy instead overwrites per rule 1.)
